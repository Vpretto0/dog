...
### SDL
def convert_to_onebyte(image_threebyte):
    #conv = numpy.asarray(image_threebyte, (480, 640))
    #numpy.asarray(Image.open(io.BytesIO(image.shot.image.data)))
    #image_threebyte = image_threebyte[0]
    #image_threebyte.shot.image.data = io.BytesIO(image_threebyte.shot.image.data[0])
    
    #print("Type:",type(numpy.asarray(Image.open(io.BytesIO(image_threebyte.shot.image.data)))))
    #flat = numpy.asarray(Image.open(io.BytesIO(image_threebyte.shot.image.data))).flatten()
    #print("Flat-shape:", flat.shape)
    
    #image_onebyte = numpy.asarray(Image.open(io.BytesIO(image_threebyte.shot.image.data)))[:, :, 0]
    flattened = numpy.asarray(Image.open(io.BytesIO(image_threebyte.shot.image.data)))[:, :, 0]
    print("Flat-shape:", flattened.shape)
    return image_threebyte
    #return conv ### TODO
...
def stitch(robot, options):
    ...
    ### SDL: Clamp to either 3 or 1 (Pixel-format)
    color_requested = (options.pixel_format == "PIXEL_FORMAT_RGB_U8") or (options.pixel_format == "PIXEL_FORMAT_RGB_U16")
        ...
        if images_future is not None and images_future.done():
            try:
                ...
                for image in images:
                    ...
                    ### SDL: Original formula used the one byte grayscale (Pixel-format = 1). We need to stick with that for most things, but overwrite final image value using color 3 byte (Pixel-format = 3)
                    image_onebyte = None
                    image_threebyte = None
                    if not color_requested:
                        image_onebyte = image
                        image_threebyte = None
                    else:
                        image_onebyte = convert_to_onebyte(image) ### TODO
                        image_threebyte = image ### TODO                       
                    #print("Pixel-format:", image.shot.image.pixel_format)


                    if image.source.name == 'frontright_fisheye_image':
                        front_right_image = image_threebyte ### SDL
                        front_right = ImagePreppedForOpenGL(image)
                    elif image.source.name == 'frontleft_fisheye_image':
                        front_left_image = image_threebyte ### SDL
                        front_left = ImagePreppedForOpenGL(image)
                ### SDL: Replace image with image derived from get_image.py
                if color_requested:
                    # img_cv = Image.fromarray(image_to_opencv(front_right_image))
                    # img_gl = Image.fromarray(front_right.image)
                    # if img_cv != None and img_gl != None:
                    #     img_cv.save("cv_test1.png")
                    #     img_gl.save("gl_test1.png")
                    print("Image-type", type(front_right.image))                 
